...
export async function createAntropometri(req, res) {
    try {
        ...
        const data = await antropometriService.createAntropometri(Number(value.kunjungan_id), value.tinggi, value.berat, value.lingkar_kepala ? Number(value.lingkar_kepala) : null, value.umur_bulan ?? null, value.catatan || null, req.user.id);
...
export async function getAntropometriByPasien(req, res) {
    try {
...
export async function getAntropometriReport(req, res) {
    try {
        const filters = {
            poli: req.query.poli,
...
//# sourceMappingURL=antropometriController.js.map