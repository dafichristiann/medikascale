...
export async function getKunjunganList(filters) {
    let query = `
...
export async function getKunjunganById(id) {
    const result = await pool.query(`SELECT k.id, k.no_antrian, k.tanggal_kunjungan, k.poli, k.layanan, k.status_antrian, k.prioritas,
            k.created_at, k.updated_at,
...
export async function updateKunjunganStatus(id, status, prioritas, changedByUserId) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const current = await client.query('SELECT status_antrian, prioritas FROM kunjungan WHERE id = $1 FOR UPDATE', [id]);
        ...
        await client.query('UPDATE kunjungan SET status_antrian = $1, prioritas = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3', [status, newPrioritas, id]);
        await client.query('INSERT INTO antrian_log (kunjungan_id, status_lama, status_baru, changed_by) VALUES ($1, $2, $3, $4)', [id, oldStatus || null, status, changedByUserId]);
...
export async function getAantrianLog(kunjunganId) {
    const result = await pool.query(`SELECT al.id, al.kunjungan_id, al.status_lama, al.status_baru, al.changed_at,
            u.username as changed_by_username
     FROM antrian_log al
     LEFT JOIN users u ON al.changed_by = u.id
     WHERE al.kunjungan_id = $1
     ORDER BY al.changed_at ASC`, [kunjunganId]);
    return result.rows;
}
//# sourceMappingURL=aantrianService.js.map