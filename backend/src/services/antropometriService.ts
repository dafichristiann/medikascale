...
export async function createAntropometri(
  kunjunganId: number,
  tinggi: number,
  berat: number,
  ...
  if (umur === null || umur === undefined) {
    const kunj = await pool.query(
      `SELECT p.tanggal_lahir FROM kunjungan k JOIN pasien p ON p.id = k.pasien_id WHERE k.id = $1`,
      [kunjunganId]
    );
  ...
  const result = await pool.query(
    `INSERT INTO antropometri (kunjungan_id, tinggi, berat, lingkar_kepala, umur_bulan, catatan, created_by_user_id)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [kunjunganId, tinggi, berat, lingkarKepala, umur, catatan, createdByUserId]
  );
  return result.rows[0];
}

export async function getAntropometriByPasien(pasienId: number) {
  const result = await pool.query(
    `SELECT a.*, k.tanggal_kunjungan, u.username as created_by_username
     FROM antropometri a
...
export async function getAntropometriReport(filters: any) {
  let query = `
  ...
  return {
    ...
    'at-risk': 0,
    malnutrition: 0,
    data: data.map((row: any) => ({ ...row, status: 'normal' })),
  };
}
